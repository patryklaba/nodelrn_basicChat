...
exports.listen = function(server) {
  ...
  function handleMessageBroadcasting(socket) {
    socket.on('message', (message) => {
      try {
        ...
        console.log('Error with broadcasting message', error);
      }
    });
  }
  ...
  function joinRoom(socket, roomName) {
    ...
    socket.emit('server:info', { text: `You have just joined ${currentRoom[socket.id]} room` });
  
    // inform whole room about new user joining
    socket.broadcast.to(roomName).emit('server:info', {
      text: `${nickNames[socket.id]} joined the room ${roomName}`
    })
    ...
    const usersInRoom = io.clients( (err, clients) => {
      ...
      socket.emit('server:info', {text: infoMsg});
    })
  }
  ...
  function handleNickChange(socket) {
    ...
    socket.on('client:ChangeNick', (newNick)=>{
      ...
      if(isValid) {
        console.log('i want to change your nick');
  ...
  function handleClientDisconnect(socket) {
    socket.on('disconnect', ()=> {
      console.log(`User ${nickNames[socket.id]} left the room`);
      let i = namesUsed.indexOf(nickNames[socket.id]);
